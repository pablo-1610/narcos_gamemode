window.APP = {
  ...
  mounted() {
    post('http://narcos_gamemode/loaded', JSON.stringify({}));
  ...
  methods: {
    ...
    send(e) {
      if(this.message !== '') {
        post('http://narcos_gamemode/chatResult', JSON.stringify({
    ...
    hideInput(canceled = false) {
      if (canceled) {
        post('http://narcos_gamemode/chatResult', JSON.stringify({ canceled }));
      }
      this.message = '';
