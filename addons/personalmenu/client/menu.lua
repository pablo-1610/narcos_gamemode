...
Narcos.netRegisterAndHandle("idCardShown", function(otherPlayer, infos)
    local otherPed = GetPlayerPed(GetPlayerFromServerId(otherPlayer))
    local mugshot, mugshotStr = NarcosClient.PlayerHeler.getPedMugshot(otherPed)
    Narcos.toInternal("showAdvancedNotification", "Carte d'identitié", ("~o~%s %s ~s~(~o~%s ans~s~)"):format(infos[1], infos[2], infos[3]), "", mugshotStr, 7, false)
end)
