...
local function LoadDefaultModel(malePed, cb)
    ...
    local modelByState = {[false] = "mp_f_freemode_01", [true] = "mp_m_freemode_01"}

    local model = GetHashKey(modelByState[malePed])
    RequestModel(model)
    ...
    Narcos.newThread(function()
        ...
        Narcos.toInternal('skinchanger:modelLoaded')
    end)
end
...
Narcos.netHandle('skinchanger:loadDefaultModel', function(loadMale, cb)
    LoadDefaultModel(loadMale, cb)
end)

Narcos.netHandle('skinchanger:getData', function(cb)
...
Narcos.netHandle('skinchanger:change', function(key, val)
    Character[key] = val

    if key == 'sex' then
        Narcos.toInternal('skinchanger:loadSkin', Character)
    else
        ApplySkin(Character)
    end
...
Narcos.netHandle('skinchanger:getSkin', function(cb)
    cb(Character)
end)
...
Narcos.netHandle('skinchanger:loadSkin', function(skin, cb)
    if skin['sex'] ~= LastSex then
        LoadSkin = skin

        if skin['sex'] == 0 then
            Narcos.toInternal('skinchanger:loadDefaultModel', true, cb)
        else
            Narcos.toInternal('skinchanger:loadDefaultModel', false, cb)
        end
    else
        ApplySkin(skin)
...
Narcos.netHandle('skinchanger:loadClothes', function(playerSkin, clothesSkin)
